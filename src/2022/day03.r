...
rows <- input %>%
    strsplit(., "\n") %>%
    .[[1]] %>%
    map(function(row) strsplit(row, "")[[1]])

map_char_to_ascii_code <- function(char) sapply(char, utf8ToInt)

map_to_item_priorities <- function(items) items %>% 
    map(map_char_to_ascii_code) %>%
    map(function(char_code) if (char_code >= 97) char_code - 96 else char_code - 38)

sum_priorities <- function(items) items %>%
    map(map_to_item_priorities) %>%
...
rows %>%
    map(function(row) list(head(row, length(row) %/% 2), tail(row, length(row) %/% 2))) %>%
    map(function(parts) Reduce(intersect, parts)) %>%
    sum_priorities %>%
    print
